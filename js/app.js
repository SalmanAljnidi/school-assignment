...
function buildSchedule(){
  ...
  // خيار: إلغاء القيود (لتقليل الفراغات قدر الإمكان) مع الإبقاء على منع تعارض المعلم
  const relax = Boolean($("chkRelax")?.checked);
  ...
  function canPlace(teacherOcc, sectionKey, grid, lens, day, period, lesson){
    if(grid[day][period] !== "") return false;
    if(lesson.teacherId && teacherOcc[lesson.teacherId][day][period] !== null) return false;

    // عند تفعيل "إلغاء القيود" نكتفي بشرط عدم تعارض المعلم ووجود خانة فارغة
  ...
  function buildOnce(){
      ...
      for(const lesson of lessons){
        ...
        for(let d=0;d<5;d++){
          for(let p=0;p<7;p++){
            ...
            if(!canPlace(teacherOcc, s.key,grid,lens,d,p,lesson)) continue;
    ...
    return {teacherOcc, sectionSchedules, unscheduled};
  }

  // Retry عدة مرات لاختيار أفضل نتيجة (أقل تعذر)
  let bestRun=null;
  const ATTEMPTS = relax ? 25 : 10;
  for(let i=0;i<ATTEMPTS;i++){
    const run = buildOnce();
    if(!bestRun || run.unscheduled.length < bestRun.unscheduled.length){
      bestRun = run;
      if(bestRun.unscheduled.length===0) break;
    }
  }

  const teacherOcc = bestRun.teacherOcc;
  const sectionSchedules = bestRun.sectionSchedules;
  const unscheduled = bestRun.unscheduled;
...
function renderUnscheduledReport(){
  ...
  if(list.length===0){ wrap.innerHTML = `<div class="badge ok">لا توجد حصص متعذرة</div>`; return; }
  wrap.innerHTML = renderUnscheduledTable(list);
}
...
function renderStandbyAll(){
  ...
  // per-teacher standby slots list
  const slotLists = ts.map(t=>{
    const slots=[];
    const lens = state.schedule.globalLens || [7,7,7,7,7];
    for(let di=0; di<DAYS.length; di++){
      for(let pi=0; pi<7; pi++){
        if(pi>=lens[di]) continue;
        if(t.grid[di][pi]===SUBJECTS.STANDBY){
          slots.push(`${DAYS[di]} - الحصة ${pi+1}`);
        }
      }
    }
    const count=slots.length;
    return `<div class="schedule-block">
      <div class="row"><strong>${escapeHtml(t.teacher.name)}</strong> <span class="badge">انتظار ${count}</span></div>
      <div class="small">${slots.length? slots.join("، ") : "لا يوجد"}</div>
    </div>`;
  }).join("");
  wrap.innerHTML = `<div class="schedule-block"><h3>الانتظار (شامل)</h3>${table}</div>` + slotLists;
}
